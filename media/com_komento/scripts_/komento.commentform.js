...
Komento.Controller(
	...
	{ return {
		init: function() {
			...
			// initialise bbcode
			// deprecated soon. use form_editor == 'bbcode instead'
			if(Komento.options.config.enable_bbcode == 1) {
				self.commentInput().markItUp($.getBBcodeSettings());
			}
			/*if(Komento.options.config.form_editor == 'bbcode') {
				self.commentInput().markItUp($.getBBcodeSettings());
			}*/
			...
			// bind length check and validation to wysiwyg editor
			/*$('#commentInput_ifr').contents().find('body').bind('keyup', function() {
		...
		validateEmail: function(email) {
			if( Komento.options.config.enable_email_regex == 1 ) {
				var regex = new RegExp( Komento.options.config.email_regex );
		...
		validateWebsite: function(website) {
			if( Komento.options.config.enable_website_regex == 1 ) {
				var regex = new RegExp( Komento.options.config.website_regex );
