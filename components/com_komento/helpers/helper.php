...
class Komento
{
	...
	public static function getConfig( $component = '', $default = true )
	{
		...
		if( empty( $configs[$component] ) )
		{
			if( $default )
			{
				...
				$ini		= KOMENTO_ADMIN_ROOT . DIRECTORY_SEPARATOR . 'configuration.ini';
				$config		= Komento::getRegistry( JFile::read( $ini ) );
			...
			$stored = Komento::getRegistry( $dbConfig->params );
	...
	public static function getKonfig()
	{
		...
		{
			//load default ini data first
			$ini		= KOMENTO_ADMIN_ROOT . DIRECTORY_SEPARATOR . 'konfiguration.ini';
			$konfig		= Komento::getRegistry( JFile::read( $ini ) );
	...
	public static function getRegistry( $contents = '' )
	{
		...
		$registry = new KomentoRegistry( $contents );
	...
	public static function getUsersByGroup( $gid )
	{
		...
		{
			$db = Komento::getDBO();
			$query = 'SELECT `id` FROM `#__users` WHERE `gid` = ' . $gid;
			$db->setQuery($query);
			$userArr = $db->loadResultArray();
		}
	...
	public static function addDiscussPoint( $action , $userId = 0, $title = '' )
	{
		...
		if( $my->id != 0 )
		{
			...
			{
				DiscussHelper::getHelper( 'History' )->log( $action , $userId, $title );
			}
	...
	*/
	...
	public static function getEvents( $type, $params = array() )
	{
		Komento::import( 'class', 'events' );
		$class = KomentoEvents::getInstance();
		$events = $class->getEvents( $type, $params );
	...
	public static function getSupportedComponents()
	{
		return explode( ',', KOMENTO_SUPPORTED_COMPONENTS );
	}
