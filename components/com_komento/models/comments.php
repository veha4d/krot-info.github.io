...
class KomentoModelComments extends KomentoModel
{
	...
	private function buildWhere( $component = 'all', $cid = 'all', $options = array() )
	{
		...
		if( $cid !== 'all' && isset( $cid ) )
		{
			...
			{
				$queryWhere[] = $this->db->nameQuote( 'cid' ) . ' = ' . $cid;
			}
		}
		...
		{
			$queryWhere[] = $this->db->nameQuote( 'sticked' ) . ' = ' . $this->db->quote( $options['published'] );
		}
	...
	function getPopularComments( $component = 'all', $cid = 'all', $options = array() )
	{
		...
		$querySelect  = 'SELECT comments.*, COUNT(actions.comment_id) AS likes, 0 AS depth FROM ' . $this->db->nameQuote( '#__komento_comments' ) . ' AS comments';
		...
		if( $options['sticked'] !== 'all' )
		{
			$queryWhere[] = 'comments.sticked = ' . $this->db->quote( $options['published'] );
		}
