...
class KomentoModelSubscription extends KomentoModel
{
	...
	function checkSubscriptionExist($component, $cid, $userid = 0, $email = '', $type = 'comment')
	{
		...
		{
			$query	.= ' AND `email` = ' . $this->db->Quote($email);
		}
	...
	function remove( $subscribers = array() )
	{
		...
		if( count( $subscribers ) > 0 )
		{
			...
			$query  = 'DELETE FROM ' . $this->db->nameQuote( '#__komento_subscription' );
			$query .= ' WHERE ' . $this->db->nameQuote( 'id' ) . ' IN (' . $all . ')';
			$this->db->setQuery( $query );
	...
	{
		$query = 'SELECT DISTINCT ' . $this->db->namequote( 'component' ) . ' FROM ' . $this->db->namequote( '#__komento_subscription' ) . ' ORDER BY ' . $this->db->namequote( 'component' );
		$this->db->setQuery($query);
	...
	function _buildQuery()
	{
		...
		$querySelect  = 'SELECT * FROM ' . $this->db->namequote( '#__komento_subscription' );
		...
		if( $filter_type != '*' )
		{
			$queryWhere[] = 'type = ' . $this->db->quote( $filter_type );
		}
